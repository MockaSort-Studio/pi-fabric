...
- Worktree path gotcha (cost us four aborted agent rounds): the harness names the worktree directory after ITSELF, not the target repo — even a pi-fovea worktree lands under `$TMPDIR/pi-fabric-worktrees/<id>`. Repo identity lives in the registered worktree list and branch tip, not the directory name. Never instruct an agent to verify its location by directory-name pattern; verify by repo instead (`git rev-parse --show-toplevel` + `git log -1 --format=%H` against the expected base, or `git remote get-url origin`).
