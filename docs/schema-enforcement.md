...
It blocks `pi.edit`, `pi.write`, `pi.bash`, all agent/actor actions, mesh and state writes or execution, `compact.request`, `compact.cancel`, and `components.reload`, MCP, captured extensions, and every external provider regardless of its declared risk. Declarative component entries are retained in configuration but are not activated in enforce sessions; registered definitions remain visible to diagnostics. A provider claiming `risk: "read"` does not bypass this exact-reference policy. Guard failures are recorded in the existing typed execution trace with `failureStage: "guard"`.
