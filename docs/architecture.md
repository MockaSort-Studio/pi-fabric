...
The [component plane](components.md) sits above the registry; its [executable calculus](component-calculus.md) states the corresponding laws and author obligations. Definitions declare exact action requirements and optional provided services; the loader resolves a versioned, descriptor-hashed capability view before activation. Providers mounted during activation remain staged until guarded effects complete and the target is reverified. Publication is atomic. Replacement retires the old generation, unloads dependents, and keeps that generation alive for retained views and in-flight calls until quiescence. Yield boundaries admit early diversion, transition epochs prevent stale publication, declared provisions are disjoint, and parent-owned child fibers unwind before their parent. The dashboard renders this requirement graph as a separate component group rather than folding it into participant ownership topology. Effect scopes provide asynchronous LIFO rollback and quarantine cleanup failures.
