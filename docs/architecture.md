...
Fabric keeps the complete authored TypeScript, named strings, result, and audit trace in the session. On the first provider request after a successful `fabric_exec` call, the provider-facing `context` projection replaces its arguments with a bounded summary of audited operations. Projecting at this boundary preserves the provider's stable cached prefix because the completed call was not part of the preceding request. Failed calls keep their original TypeScript for repair. Tool-call IDs, outcomes, paths, command previews, and model-visible result evidence remain available. This projection is deterministic and non-destructive; resuming, branching, auditing, and the TUI continue to use the original session entries.
