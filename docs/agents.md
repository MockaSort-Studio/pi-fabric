...
Prewalk adds no system-prompt instructions. Its hidden continuation is queued only after a matching mutation boundary; it is not an open-ended per-turn nudge. Turns that settle without a handoff never disarm prewalk: the arm is consumed only when a matching mutation claims it (in-place switch or trajectory spawn), by a completed explicit `agents.handoff()`, or by `/fabric prewalk --off`. The settled turn's captured task text is dropped so the next prompt is recaptured fresh. Both modes require full code mode and are unavailable in Schema enforce mode.
